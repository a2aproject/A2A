...
For the first time a message is sent, agent replies back with a `contextId` (generated by the server). If the agent responded with a task, then it will also have a `taskId` (generated by the server for a new task). Clients can send subsequent messages and attach the same `contextId` to indicate to the agent, that they are continuing their previous interaction within the same context. Client can optionally attach the `taskId` to a subsequent message to indicate that the message is in continuation of that specific task.
