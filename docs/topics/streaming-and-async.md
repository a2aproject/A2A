...
- **Stream Termination:** When a task reaches a terminal state (COMPLETED, FAILED, CANCELLED, or REJECTED), the server closes the stream and sends no further updates.
