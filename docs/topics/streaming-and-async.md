...
For tasks that produce incremental results (like generating a long document or streaming media) or provide ongoing status updates, A2A supports real-time communication via streaming. The protocol defines streaming behavior in a transport-agnostic way: the standard bindings use Server-Sent Events (SSE) and gRPC server streaming as defaults, but agents **MAY** advertise support for alternative streaming transports (such as WebSockets, PubNub, Kafka, or MQTT) via the `streamingTransports` field in their [`AgentInterface`](../specification.md#AgentInterface).

The following key features detail how streaming is implemented and managed within the A2A protocol:
...
- **Streaming Transport Discovery:** Clients discover the available streaming transports by inspecting the `streamingTransports` field of the agent's [`AgentInterface`](../specification.md#AgentInterface). When this field is absent, the default transport for the protocol binding is used (SSE for JSON-RPC and HTTP+JSON, server streaming for gRPC). When present, clients **SHOULD** select the first transport they support.
...
- **Server Response and Connection (SSE default):** When using the default SSE transport, the server responds with an HTTP 200 OK status and a `Content-Type: text/event-stream`. This HTTP connection remains open for the server to push events to the client. When using an alternative transport, the connection mechanism is defined by that transport (for example, subscribing to a pub/sub channel).

- **Event Structure and Types:** The server sends [`StreamResponse`](../specification.md#323-stream-response) events over the stream. For example, when using the default SSE transport, each event's `data` field contains a JSON-RPC 2.0 Response object whose `result` is a `StreamResponse`. Regardless of transport, the `StreamResponse` contains:
...
- **Stream Termination:** When a task reaches a terminal or interrupted state (e.g., `COMPLETED`, `FAILED`, `CANCELED`, `REJECTED`, or `INPUT_REQUIRED`), the server closes the stream and sends no further updates. The mechanism for closing depends on the transport (e.g., closing the SSE connection, sending a final message on a pub/sub channel).

- **Resubscription:** If a client's streaming connection breaks prematurely while a task is still active, the client is able to attempt to reconnect to the stream using the `SubscribeToTask` RPC method. The reconnection mechanism depends on the transport.

### When to Use Streaming

Streaming is best suited for:
...
Different streaming transports offer different trade-offs:

- **SSE (default):** Simple, HTTP-based, widely supported. Requires persistent HTTP connection. Best for browser-based and server-to-server scenarios.
- **Pub/sub transports (e.g., PubNub, Kafka, MQTT):** Decouple producer and consumer, support fan-out and offline message catch-up. Best for mobile clients, multi-subscriber scenarios, and environments where persistent HTTP connections are impractical.
- **WebSockets:** Bidirectional, low-overhead. Best for interactive applications requiring both client-to-server and server-to-client real-time communication.
...
- [`AgentInterface`](../specification.md#AgentInterface) and [`StreamingTransport`](../specification.md#StreamingTransport)
