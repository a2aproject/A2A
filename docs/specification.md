...
- **Context:** An optional identifier to logically group related tasks.
...
- **Client generated context id**: If `AgentCard.canReceiveClientGeneratedContextId` is `false` or not present, Agent **MUST** reject requests with `contextId` that was not previously generated by the Agent. If it's set tu `true` Agent **MUST** accept the client-generated `contextId` and use it to group `Tasks` and `Messages`.
