...
- **Blocking (`blocking: true`)**: The operation MUST wait until the task reaches a terminal or interrupted state (`COMPLETED`, `FAILED`, `CANCELED`, `REJECTED`) or an interrupted state (`input_required`, `auth_required`) before returning. The response MUST include the latest task state with all artifacts and status information.
